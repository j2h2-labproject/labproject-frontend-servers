...
module.exports = {
	...
	start: function(config, onstart, onstop, callback) {
		...
		if (config.vm_servers.transport !== undefined) {
			var transport_name = config.vm_servers.transport;
			var transport = null;
			...
			logger.log("notice", "No transport in config, starting without backend server");
			start_frontends(null);
		}
