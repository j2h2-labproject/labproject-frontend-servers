...
function register_user_commands(vorpal_inst, session_id) {
  vorpal_inst
    .command('id', 'List your user and group info')
    .action(function(args, callback) {
      var v_this = this;
      usergroup_controller.id(session_id, function(error, result) {
        if (!error) {
          v_this.log('Username: ' + result.username + '\nGroups:');
          for (var i = 0; i < result.groups.length; i++) {
            v_this.log('\t' + result.groups[i]);
          }
        } else {
