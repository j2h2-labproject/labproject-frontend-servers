...
function virtual_machine(uuid, server_client) {
	...
	self.state = {
		...
		undefine: function(callback) {
			server_client.call(Private.vm_server, "delete_vm", {uuid: Private.uuid}, function(s_error, result) {
				if (!s_error) {
		...
		start: function(callback) {
			database.insert('allocated_devices', {uuid: Private.uuid}, function(i_error, result) {
				...
				if (!i_error) {
					server_client.call(Private.vm_server, "start_vm", {uuid: Private.uuid}, function(s_error, result) {
						if (!s_error) {
							callback(null, true);
						} else {
							callback(s_error, null);
					...
					if (i_error.name === "MongoError" && i_error.code === 11000) {
						callback(new Error("Device is already allocated"), null);
					} else {
						callback(i_error, null);
					}
				}
		...
		stop: function(callback) {
			database.remove('allocated_devices', {"uuid": Private.uuid}, function(error, result) {
				if (!error) {
					server_client.call(Private.vm_server, "stop_vm", {uuid: Private.uuid}, function(s_error, result) {
						if (!s_error) {
							callback(null, true);
						} else {
							callback(s_error, null);
		...
		is_running: function(callback) {
			server_client.call(Private.vm_server, "vm_is_running", {uuid: Private.uuid}, function(s_error, result) {
				if (!s_error) {
					callback(null, result);
				} else {
					callback(s_error, null);
