...
function vm_server(name, server_client) {
	...
	// Verify the networking of the VM server
	self.verify_networking = function(callback) {
		// Verify the external switch
		Private.server_client.call(Private.name, "switch_exists", {switch_id: "external"}, function(s_error, result) {
			if (!s_error) {
				callback(null, result);
			} else {
				console.log("error", s_error);
	...
	self.reboot_maintenance_network = function(callback) {
		...
		var maintenance_switch_name = "maintenance";
