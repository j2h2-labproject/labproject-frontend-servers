...
function iso_query(query, callback) {
    database.find('isos', query, {}, function(d_error, lab_list) {
        if (!d_error) {
...
module.exports = {
    ...
    list_isos: function(callback) {
        iso_query({}, callback);
    },
    list_isos_by_tags: function(tags, callback) {
        if (!Array.isArray(tags)) {
            callback(new Error("Tags are not an array"));
            return;
        }
        for (var i = 0; i < tags.length; i++) {
            tags[i] = sanitize.simple_string(tags[i]);
        }
        iso_query({"tags": {"$all": tags}}, callback);
    },
    // Create a new iso
...
function iso(iso_id, server_client) {
    ...
    self.get_tags = function() {
        if (Private.tags.length > 0) {
            return Private.tags.slice(0);
        } else {
            return [];
        }
    };
    ...
    self.add_tag = function (tag) {
        tag = sanitize.simple_string(tag);
        if (Private.tags.indexOf(tag) == -1) {
            Private.tags.push(tag);
        }
    }

    self.remove_tag = function(tag) {
        tag = sanitize.simple_string(tag);
        var loc = Private.tags.indexOf(tag);
        if (loc > -1) {
            Private.tags.splice(loc, 1);
        }
    }
}
