  ...
  self.get_devices = function() {
	...
	self.get_interface = function(callback) {
		...
		database.find("interfaces", {"interface_name": interface_match}, {}, function(d_error, results) {
			...
			allocate_interface(results.length, function(i_error, interface_name) {
				if (!i_error) {
	...
	self.remove_interface = function(interface_name, callback) {
		...
		if (interface_match.test(interface_name)) {
			database.remove("interfaces", {"interface_name": interface_name}, function(d_error, results) {
				if (!d_error) {
					callback(null, true);
				} else {
					callback(d_error, null);
	...
	function allocate_interface(last, callback) {
		var interface_name =  "lpif" + Private.interface_group + "." + last;
		database.insert('interfaces', {"interface_name": interface_name}, function(i_error, result) {
			if (!i_error) {
				callback(null, interface_name);
			} else {
				if (i_error.name === "MongoError" && i_error.code === 11000) {
					allocate_interface(last + 1, callback);
				} else {
					callback(i_error, null);
				}
			}
	...
	function check_group(pos, group_id, callback) {
		if (pos >= Private.current_servers.length) {
			...
			server_client.call(Private.current_servers[pos], "interface_exists", {devicename: "lpif" + group_id}, function(s_error, result) {
				if (!s_error) {
					if (result === true) {
						deallocate_group(pos+1, group_id, callback);
					} else {
