...
module.exports = {
	connect_clients: function(logger, hosts_config, callback) {
		...
		add_clients(logger, hosts_config, {}, function(error, client_map) {
...
function add_clients(logger, hosts_config, client_map, callback) {
	
	if (hosts_config.length == 0) {
		callback(null, client_map);
		return;
	}
	
	var current = hosts_config[0];
	...
	client_map[current.name].connect('http://' + current.host + ':' + current.port, function(error, status) {
		hosts_config.splice(0, 1);
		add_clients(logger, hosts_config, client_map, callback);
	});
